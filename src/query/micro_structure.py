...
class MicroStructureAgent:
    ...
    def get_adsorbate_symbols(self) -> List[str]:
        ...
        """
        return [
            col.split("*")[1] for col in self.df.columns if col.startswith("energy_*")
        ]
    ...
    def filter_nan_rows(self):
        ...
        logger.debug(
            f"Removed {rows_removed} rows with NaN values in {self.column_of_interest}"
        )
    ...
    def answer_query(
        self, query: str, catalysts: List[str], max_attempts: int = 3
    ) -> str:
        """
        ...
        prompt = self.generate_prompt(
            ...
            catalysts,
            miller_index,
            is_active_sites_query,
            is_summary,
            is_preferable_surface_query,
        )
        ...
        while index_attempts < max_attempts:
            try:
                ...
                logger.info(
                    f"Attempt {index_attempts+1}: Generated pandas query: \n{pandas_query}"
                )

                df = (
                    self.df.copy()
                )  # Create a local variable 'df' that refers to self.df
    ...
    def generate_prompt(
        ...
        catalysts: List[str],
        miller_index: str,
        is_active_sites_query: bool,
        is_summary: bool,
        is_preferable_surface_query: bool,
    ) -> str:
        """
    ...
    def format_response(
        ...
        if isinstance(result, pd.DataFrame):
            ...
            response += self.summarize_dataframe(
                result,
                is_active_sites_query,
                is_summary,
                adsorbate,
            ...
            response += result.to_string() + "\n\n"
            response += self.summarize_series(
                result,
                is_active_sites_query,
                adsorbate,
                is_preferable_surface_query,
    ...
    def summarize_dataframe(
        ...
        if is_preferable_surface_query:
            ...
            if "bulk_composition" in df.index.names:
                unique_compositions = df.index.get_level_values(
                    "bulk_composition"
                ).nunique()
            ...
            if "site_composition" in df.columns and catalysts:
                df["catalyst_match"] = df["site_composition"].apply(
                    lambda x: any(cat in x for cat in catalysts)
                )
                preferable_surface = (
                    "near" if df["catalyst_match"].any() else "far from"
                )
    ...
    def summarize_series(
        self, series: pd.Series, is_active_sites_query: bool, adsorbate: str
    ) -> str:
        """
        ...
        if series.name == "reward":
            ...
            avg_value = series.mean()
            summary += (
                f"- The average adsorption energy for {adsorbate} is {avg_value:.2f}.\n"
            )
        else:
    ...
    def is_active_sites_query(self, query: str) -> Tuple[bool, bool]:
        ...
        """
        is_active_sites = (
            "active site" in query.lower() or "active sites" in query.lower()
        )
